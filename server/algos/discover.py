...
def handler(cursor: Optional[str], limit: int, requester_did: str) -> dict:
    ...
    stmt = sqlalchemy.select(Post).filter(Post.discoverable).where(Post.did.not_in(all_followed_dids)).order_by(Post.created_at.desc()).limit(limit)
    posts = session.scalars(stmt).all()
    ...
    if cursor:
        ...
        posts = [post for post in posts if (post.created_at == indexed_at and post.cid < cid) or post.created_at < indexed_at]
